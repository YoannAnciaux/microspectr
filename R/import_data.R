...
import_excel <- function(path,
  ...
  tbl_name_time <- filter(data, grepl("Time", Content)) %>%
  ...
  if(is.list(name_repair)) {
    ...
    tbl_name_time <- tbl_name_time %>%
      mutate(raw_Signal = Signal,
             Signal = str_replace_all(Signal, pattern = set_names(replacement, pattern)))
  } else {
    tbl_name_time <- tbl_name_time %>%
      mutate(raw_Signal = Signal,
             Signal = gsub(".*\\((.*)\\).*", "\\1", Signal)) # pattern to extract the names that are inside the parentheses
  }
